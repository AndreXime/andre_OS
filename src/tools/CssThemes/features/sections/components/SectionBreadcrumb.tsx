...
function BreadcrumbLink({
	...
	if (current || !onClick) {
		return (
			<span
				className={cn("text-sm", current ? "font-medium text-ct-foreground" : "text-ct-muted-foreground")}
...
export function SectionBreadcrumb({
	...
	return (
		<nav aria-label="Breadcrumb" className="mb-6 border-b border-ct-border pb-4">
			<ol className="flex flex-wrap items-center gap-1.5 text-sm">
				<li className="flex items-center gap-1.5">
					<BreadcrumbLink onClick={category ? onNavigateCatalog : undefined} current={!category}>
						Seções
					</BreadcrumbLink>
				</li>
				{category && (
					<li className="flex items-center gap-1.5">
						...
						<BreadcrumbLink onClick={variation ? onNavigateCategory : undefined} current={!variation}>
							{category.name}
						</BreadcrumbLink>
					</li>
				)}
