...
import { cssThemes$, resetTokens, updateColor } from "../../store/themeStore";
		...
		>
			<div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
				{(Object.keys(COLOR_LABELS) as Array<keyof ColorTokens>).map((key) => (
					<label key={key} className="flex flex-row gap-2 rounded-ct-md border border-ct-border p-3">
						<div className="flex items-center gap-2">
							...
							<div className="min-w-0">
								<span className="block text-xs font-medium">{COLOR_LABELS[key]}</span>
								<code className="text-[0.65rem] text-ct-muted-foreground">{key}</code>
							</div>
						</div>
						<Input value={tokens.colors[key]} onChange={(e) => updateColor(key, e.target.value)} />
					</label>
				))}
			</div>
			<details className="mt-4">
				<summary className="cursor-pointer text-sm font-medium">Exportar CSS variables</summary>
