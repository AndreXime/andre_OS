...
export function ThemePicker() {
	...
	const [appearanceFilter, setAppearanceFilter] = useState<ThemeAppearanceFilter>("all");
	const [page, setPage] = useState(0);

	const filteredPresets = useMemo(() => filterPresetsByAppearance(THEME_PRESETS, appearanceFilter), [appearanceFilter]);

	const totalPages = Math.max(1, Math.ceil(filteredPresets.length / THEMES_PER_PAGE));

	useEffect(() => {
		setPage(getPresetPageIndexInList(filteredPresets, activePresetId, THEMES_PER_PAGE));
	...
	const visiblePresets = filteredPresets.slice(pageStart, pageStart + THEMES_PER_PAGE);

	return (
		<Panel title="Temas" description="Escolha um preset e edite os tokens do tema selecionado.">
			<SegmentedControl
			...
			{visiblePresets.length > 0 ? (
				...
				<p className="text-sm text-ct-muted-foreground">Nenhum tema encontrado para este filtro.</p>
			)}

			{totalPages > 1 && (
				<div className="mt-4 flex items-center justify-between gap-3">
					<Button variant="ghost" size="sm" disabled={page === 0} onClick={() => setPage((current) => current - 1)}>
						Anterior
					</Button>
