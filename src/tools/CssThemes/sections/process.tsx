        ...
        <div className="mt-12 grid grid-cols-1 gap-8 md:grid-cols-3">
          {ACME.process.map((item) => (
            <article key={item.step} className="min-w-0">
              ...
              <h3 className="mt-4 font-ct-heading text-lg font-semibold text-ct-foreground">
                {item.title}
              </h3>
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <ol className="mt-10 space-y-0">
          {ACME.process.map((item) => (
            <li
              key={item.step}
              className="relative flex gap-6 border-l-2 border-ct-primary/30 pb-10 pl-8 last:pb-0"
            >
    ...
    <section className="border-y border-ct-border bg-ct-background px-6 py-16">
      <div className="mx-auto grid max-w-6xl grid-cols-1 gap-10 lg:grid-cols-[1fr_1.2fr] lg:items-start">
        <div>
          ...
          <p className="mt-3 text-ct-muted-foreground">
            Get up and running in minutes, not days.
          </p>
        ...
        <div className="grid gap-4">
          {ACME.process.map((item) => (
            <div
              key={item.step}
              className="flex gap-4 rounded-ct-lg border border-ct-border p-5"
            >
              <span className="font-ct-heading text-2xl font-bold text-ct-primary/30">
                {item.step}
              </span>
              <div>
