        ...
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {ACME.features.map((feature) => (
            <article key={feature.num} className="min-w-0">
              <span className="font-ct-heading text-sm font-bold text-ct-primary">
                {feature.num}
              </span>
              <h3 className="mt-2 font-ct-heading text-lg font-semibold text-ct-foreground">
                {feature.title}
              </h3>
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <ul className="mt-10 divide-y divide-border border-y border-ct-border">
          {ACME.features.map((feature) => (
            <li
              key={feature.num}
              className="grid grid-cols-1 gap-3 py-6 sm:grid-cols-[4rem_minmax(0,1fr)] sm:gap-6"
            >
              <span className="font-ct-heading text-2xl font-bold text-ct-primary/40">
                {feature.num}
              </span>
              <div>
...
export function Features3() {
  ...
  return (
    <section className="bg-ct-background px-6 py-20">
      <div className="mx-auto max-w-6xl">
        <h2 className="mb-10 font-ct-heading text-3xl font-bold text-ct-foreground">
          Everything you need to succeed
        </h2>
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          {featured && (
            <article className="rounded-xl border border-ct-primary/30 bg-ct-primary/5 p-8 lg:row-span-2">
              ...
              <h3 className="mt-3 font-ct-heading text-2xl font-bold">
                {featured.title}
              </h3>
