	...
	return (
		<section className="border-y border-ct-border bg-ct-muted/50 px-6 py-12">
			<div className="mx-auto flex max-w-6xl flex-col divide-y divide-border">
				{ACME.stats.map((stat) => (
					<div key={stat.label} className="flex flex-col gap-2 py-6 sm:flex-row sm:items-center sm:justify-between">
						<div>
							<p className="font-ct-heading text-3xl font-bold text-ct-foreground">{stat.value}</p>
							<p className="text-sm font-medium text-ct-foreground">{stat.label}</p>
						</div>
	...
	return (
		<section className="bg-ct-background px-6 py-16">
			<div className="mx-auto max-w-4xl text-center">
				...
				<h2 className="mt-2 font-ct-heading text-3xl font-bold text-ct-foreground">Trusted at scale</h2>
				<div className="mt-10 grid grid-cols-2 gap-8 lg:grid-cols-4">
					{ACME.stats.map((stat) => (
						<div key={stat.label}>
							<p className="font-ct-heading text-2xl font-bold text-ct-primary sm:text-3xl">{stat.value}</p>
