			...
			<div className="mx-auto max-w-6xl">
				<div className="grid grid-cols-2 gap-8 sm:grid-cols-4 lg:grid-cols-5">
					<div className="col-span-2 sm:col-span-4 lg:col-span-1">
						...
						<p className="mt-3 text-sm text-ct-muted-foreground">{ACME.footer.tagline}</p>
					</div>
					{ACME.footer.columns.map((col) => (
						<div key={col.title}>
							<h3 className="text-sm font-semibold text-ct-foreground">{col.title}</h3>
							<ul className="mt-3 space-y-2">
								{col.links.map((link) => (
									<li key={link}>
										<a href="/" className="text-sm text-ct-muted-foreground transition-colors hover:text-ct-foreground">
											{link}
										</a>
									</li>
								))}
		...
		<footer className="bg-ct-muted px-6 py-12">
			<div className="mx-auto flex max-w-6xl flex-col gap-8 sm:flex-row sm:items-start sm:justify-between">
				<div>
					...
					<p className="mt-2 max-w-xs text-sm text-ct-muted-foreground">{ACME.footer.tagline}</p>
				</div>
				<div className="grid grid-cols-2 gap-8 sm:grid-cols-4">
					{ACME.footer.columns.map((col) => (
						<div key={col.title}>
							<h3 className="text-xs font-semibold uppercase tracking-wider text-ct-muted-foreground">{col.title}</h3>
							<ul className="mt-3 space-y-2">
			...
			<p className="mx-auto mt-10 max-w-6xl text-xs text-ct-muted-foreground">© 2025 {ACME.brand}</p>
		</footer>
	);
}
